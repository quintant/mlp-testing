...
def create_parallel_models(
    ...
    compile: bool = False,
    no_split: bool = True,
) -> Tuple[AutoencoderKL, UNet2DConditionModel, CLIPTextModel, int, int, int]:
    """
    ...
    for model in [vae, text_encoder]:
        ...
        if model is not None:
            ...
            model = torch.nn.DataParallel(
                model, device_ids=list(range(curr_device, curr_device + 1))
            )
            curr_device += 1
    if unet is not None:
        ...
        unet = torch.nn.DataParallel(
            unet, device_ids=list(range(num_devices))[curr_device:]
        )
...
def main(args: argparse.Namespace):
    ...
    if args.dataparallel:
        ...
        vae, unet, text_encoder, vae_device, text_encoder_device, unet_device = (
            create_parallel_models(
                vae, unet, text_encoder, compile=args.compile, no_split=args.no_split
            )
        )
